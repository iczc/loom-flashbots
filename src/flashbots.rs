use crate::client::{
    make_signed_body, BundleRequest, BundleTransaction, FlashbotsMiddleware,
    ...
    P: Provider<T, Ethereum> + Send + Sync + Clone + 'static,
{
    ...
    pub fn new_no_sign(provider: P, url: &str) -> Self {
        let flashbots_client =
            FlashbotsClient::create_flashbots_no_signer_middleware(provider, url);
    ...
    fn create_flashbots_middleware(provider: P, url: &str) -> FlashbotsMiddleware<P, T> {
        let flashbots: FlashbotsMiddleware<P, T> =
            FlashbotsMiddleware::new(Url::parse(url).unwrap(), provider);
    ...
    fn create_flashbots_no_signer_middleware(provider: P, url: &str) -> FlashbotsMiddleware<P, T> {
        let flashbots: FlashbotsMiddleware<P, T> =
            FlashbotsMiddleware::new_no_signer(Url::parse(url).unwrap(), provider);
        flashbots
    }
    ...
    pub async fn send_signed_body(&self, body: String, signature: String) -> Result<()> {
        match self
            .flashbots_middleware
            .relay()
            .serialized_request::<SendBundleResponseType>(body, Some(signature))
            .await
        {
            Ok(_resp) => {
    ...
    P: Provider<T, Ethereum> + Send + Sync + Clone + 'static,
{
        ...
        let clients: Vec<Arc<FlashbotsClient<P, T>>> = relays
            ...
            .map(|relay| {
                if relay.no_sign.unwrap_or(false) {
                    Arc::new(FlashbotsClient::new_no_sign(
                        self.provider.clone(),
                        relay.url.as_str(),
                    ))
                } else {
                    Arc::new(FlashbotsClient::new(
                        self.provider.clone(),
                        relay.url.as_str(),
                    ))
                }
        ...
        BundleTransaction: From<TX>,
    {
        ...
        let (body, signature) =
            make_signed_body(next_req_id, "eth_sendBundle", bundle, &self.signer)?;
        ...
        for client in self.clients.iter() {
            ...
            tokio::task::spawn(async move {
                ...
                let bundle_result = client_clone
                    .send_signed_body(body_clone, signature_clone)
                    .await;
                match bundle_result {
                    Ok(_) => {
                        info!(
                            "Flashbots bundle broadcast successfully {}",
                            client_clone.name
                        );
                    }
                    Err(x) => {
                        error!(
                            "Broadcasting error to {} : {}",
                            client_clone.name,
                            x.to_string()
                        );
                    }
...
mod test {
    ...
    async fn test_client_send_bundle() -> Result<()> {
        let _ = env_logger::try_init_from_env(
            env_logger::Env::default().default_filter_or("debug,flashbots=off"),
        );
        ...
        let flashbots_client =
            FlashbotsClient::new(provider.clone(), "https://relay.flashbots.net");
    ...
    async fn test_send_bundle() -> Result<()> {
        let _ =
            env_logger::try_init_from_env(env_logger::Env::default().default_filter_or("trace"));
        let node_url = Url::try_from(env::var("MAINNET_HTTP")?.as_str())?;
        ...
        let flashbots_client =
            Flashbots::new(provider.clone(), "https://relay.flashbots.net", None)
                .with_relay("https://relay.flashbots.net");
