...
mod tests {
    ...
    fn deser_response_enum() {
        let response: Response<SendBundleResponseType> =
            serde_json::from_str(r#"{"jsonrpc": "2.0", "result": 19, "id": 1}"#).unwrap();
        assert_eq!(response.id, 1);
        assert_eq!(
            response.data.into_result().unwrap(),
            SendBundleResponseType::Integer(19)
        );
    }
    ...
    fn deser_response_result_null() {
        ...
        let response: Response<SendBundleResponseType> =
            serde_json::from_str(r#"{"jsonrpc":"2.0","id":2,"result":null}"#).unwrap();
        assert_eq!(response.id, 2);
        assert_eq!(
            response.data.into_result().unwrap(),
            SendBundleResponseType::Null(None)
        );
    }
    ...
    fn deser_response_result_string() {
        ...
        let response: Response<SendBundleResponseType> =
            serde_json::from_str(r#"{"jsonrpc":"2.0","id":1,"result":"nil"}"#).unwrap();
        assert_eq!(response.id, 1);
        assert_eq!(
            response.data.into_result().unwrap(),
            SendBundleResponseType::String("nil".to_string())
        );
    }
    #[test]
    fn deser_response_result_send_bundle_response() {
        ...
        assert_eq!(
            ...
            SendBundleResponseType::SendBundleResponse(SendBundleResponse {
                bundle_hash: Some(TxHash::from(hex!(
                    "cc6c61428c6516a252768859d167dc8f5c8c8c682334a184710f898e422530f8"
                )))
            })
        )
    }
    ...
    fn deser_response_result_bundle_hash_response() {
        ...
        assert_eq!(
            response.data.into_result().unwrap(),
            SendBundleResponseType::BundleHash(TxHash::from(hex!(
                "cc6c61428c6516a252768859d167dc8f5c8c8c682334a184710f898e422530f8"
            )))
        )
    }
